...
const Liked = () => {
  const { likedItems, removeFromLiked, addToCart, loading } = useShoppingContext();
  const { user } = useAuth();
  const navigate = useNavigate();

  // Remove from favorites
  const removeFromFavorites = async (id: string) => {
    await removeFromLiked(id);
  };
  ...
  const clearFavorites = async () => {
    await Promise.all(likedItems.map(item => removeFromLiked(item._id || item.id)));
  };
                  ...
                  <div className="relative overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3">
                      <Button 
                        ...
                        onClick={() => removeFromFavorites(item._id || item.id)}
                      >
